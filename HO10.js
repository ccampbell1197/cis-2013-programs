...
var passwordCheckOld = function () 
{
    ...
    stringOutput = "";
 //if statement that determines whether the password is 8 letters or long
  if (stringPassword.length <8){
    stringOutput = ("Password too short")} else{
      for (i=0; i<arrayBadPasswords.length; i++){//for loop that checks the array for bad passwords added the phrase nineletters to the array for testing purpose
        if (stringPassword==arrayBadPasswords[i]){
          stringOutput = ("Password is banned");
          
          break;
      
        }
        else {stringOutput = ("Password is acceptable")};// outputs acceptable message if the password passes both tests
      }
    }
    $('output').value = stringOutput; //dom output
};
